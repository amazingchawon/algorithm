...
for t in range(1, T+1):
    ...
    for x in range(N-1):
        for y in range(1, N-1):
            ...
            # 4방향 탐색
            while 0 <= i < 4:
                if nx+dx[i] < 0 or nx+dx[i] >= N or ny+dy[i] < 0 or ny+dy[i] >= N:  # 다음 탐색할 곳이 경계 바깥이라 못간다! -> 한 칸 전으로 가서 탐색
                ...
                while 0 <= nx+dx[i] < N and 0 <= ny+dy[i] < N:      # 다음 탐색할 카페가 범위 안이고,
                    ...
                    # 만약 i가 0이라면 -> 해당 칸에서는 투어 불가한 것이므로 예외처리
                        ...
                        while cnt[i] > 0:
                            if i < 3 and cafes[nx + dx[i + 1]][ny + dy[i + 1]] in visited:
                                visited.remove(cafes[nx][ny])
                                nx, ny = nx - dx[i], ny - dy[i]
                                cnt[i] -= 1
                            else:
